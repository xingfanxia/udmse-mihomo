...
f=Fixture();server=admin.make_server(f,TOKEN,ROOT/'admin',0);threading.Thread(target=server.serve_forever,daemon=True).start()
...
try:
 with sync_playwright() as p:
  ...
  browser.close();print('PASS browser login, toggle, all modes, failure, degraded state, escaped labels, mobile layout and logout')
