...
本 fork 已在这台 UDM 上完成**单设备部署和管理页实测**：开关、智能分流、全局代理、直连恢复和 DNS 清理均通过，见 [部署与验收记录](docs/OPERATIONS.md)。另外有安装/权限/范围回归测试和隔离网络中的真实引擎验证。全网负载、重启、固件升级及中国大陆网络连通性仍须单独验证；不要把单设备试验当作永久全网部署验收，也不要把此项目当作 UniFi 官方支持的扩展。
...
安装默认生成独立随机管理口令，也支持显式设置的 10–128 位可打印 ASCII 密码（不含空格）；浏览器仅在内存中保存，不写入网址、本地存储或 cookie。API 验证 Bearer、Host 和同源 Origin，不开放跨域访问，也不提供执行命令或读取任意文件的接口。服务默认不设置开机自启；SSH 隧道断开后页面不可访问，但已有自动撤销计时器仍在 UDM 上执行。

### 此 Mac 自动登录

可选的 `local-dashboard.py` 在 Mac 本机保管管理凭据，经 SSH 隧道代为认证，浏览器只拿到临时的本机访问凭据。先把 UDM 管理凭据安全保存在 Mac 的 owner-only `0600` 文件中，再分别运行：

```sh
ssh -N -L 127.0.0.1:9089:127.0.0.1:9088 root@192.168.1.1
python3 local-dashboard.py --token-file ~/creds/unifi/udm-admin-token
```

打开 <http://127.0.0.1:9088> 后自动进入面板。只在信任本机用户与程序的 Mac 上启用；其他本机程序也处于这个信任范围内。它仅监听回环地址，验证 Host、Origin、浏览器请求来源和临时 Bearer；UDM 本身仍要求管理凭据。本机代理只转发固定管理接口，不提供通用网络代理。重启本机代理后，刷新页面重新连接。

页面右上角可切换浅色／深色。首次跟随系统，手动选择后保存在浏览器；保存的只有主题偏好，管理凭据不写入浏览器存储。

## 实时监测

管理页提供上下行速率、当前连接数、本次累计流量、Mihomo 进程 CPU/RSS 内存、运行时间和最近五分钟的双向速率曲线。指向曲线可查看历史样本；手机布局支持触摸查看。

统计范围是 **Mihomo 处理的流量**，包含它处理的直连、代理和后台探测，不等于全家 WAN 用量。速率单位是 B/s、KB/s、MB/s（十进制），CPU 100% 表示一个核心。累计计数来自引擎，自进程启动或引擎计数重置后累计。

单个后台采集器以约两秒间隔读取本机引擎和进程，网页只读取缓存，打开多个标签不会重复采集。慢采样、重启、计数回退或采集中断会重新建立速率基线，在曲线上留下空缺，不生成负速率或异常尖峰。代理停止、采样失败或过期时显示“—”，不把未知状态当作零流量。

历史最多保留五分钟，仅在管理服务内存中存在，管理服务重启后清空。`GET /api/telemetry` 使用同一管理口令与本机地址限制。引擎连接响应只用于提取计数和连接数量，域名、目标地址、连接 ID、订阅及凭据不会进入响应、历史或日志；原始响应超过 2 MiB 时报告不可用。
